/* okabweb.so — LD_PRELOAD shim that starts vp_project's dead-code local web/CGI server.
 *
 * O-KAM QC3 (VeePai BW6, Ingenic T23N) ships with no ESP32, and on this build the
 ...
 *
 * Freestanding: no libc is linked in. pthread_create/pthread_detach/sleep are left
 * undefined and resolved at load time against vp_project's already-loaded uClibc
...
static void *okabweb_worker(void *arg)
{
...
static void okabweb_init(void)
{
    unsigned long t;
    if (pthread_create(&t, (void *)0, okabweb_worker, (void *)0) == 0)
        pthread_detach(t);
}
