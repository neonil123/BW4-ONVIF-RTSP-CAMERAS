...
#define DEFAULT_CONF_PATH "/system/etc/okam_onvifd.conf"
...
static void set_field(okam_config_t *cfg, const char *key, const char *val) {
...
static void parse_file(okam_config_t *cfg, const char *path) {
    FILE *f = fopen(path, "r");
    if (!f)
        return;
    char line[512];
...
void config_load(okam_config_t *cfg, const char *conf_path) {
    ...
    cfg->onvif_port = 80; /* standard ONVIF port; NVRs/discovery libs default
                           ...
                           * (okabweb/livestream.cgi uses :81). Override via
    ...
    snprintf(cfg->manufacturer, sizeof cfg->manufacturer, "%s", "OKAM");
    snprintf(cfg->model, sizeof cfg->model, "%s", "QC3-T23N");
    snprintf(cfg->serial, sizeof cfg->serial, "%s", "okam-onvifd");
    ...
    const char *e;
    if ((e = getenv("OKAM_DEVPW")) && *e) snprintf(cfg->devpw, sizeof cfg->devpw, "%s", e);
    if ((e = getenv("OKAM_VUID")) && *e) snprintf(cfg->vuid, sizeof cfg->vuid, "%s", e);
}
